                                  ...
                                  await auth
                                    ...
                                    if (value.user.uid != null &&
                                      ...
                                      /*final response = await http.post(
                                          'https://api.indohobi.id/user/register_otp',
                                          body: {'key': INDOHOBI.key, 'phone_number': '${widget.phone}'});

                                      if (response.statusCode == 200) {
                                        TokenModel token = TokenModel.fromJson(json.decode(response.body));
                                        if(token != null && token.token != null){
                                          App.data.setString('token', token.token);
                                          App.data.setBool('isLogin', true);
                                          for (int i = 0; i < 3; i++) {
                                            Navigator.pop(context);
                                          }
