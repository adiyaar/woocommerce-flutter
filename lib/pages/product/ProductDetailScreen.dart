...
import 'package:nyoba/pages/product/new_products/AllNewProductsScreen.dart';
                          ...
                          ListView.builder(
                              ...
                              itemBuilder: (context, i) {
                                return AspectRatio(
                                  aspectRatio: 1 / 1,
                                  child:
                                      Image.network(productModel.images[i].src),
                                );
                              }),
                        ...
                        GestureDetector(
                          onTap: () {
                            Navigator.push(
                                context,
                                MaterialPageRoute(
                                    builder: (context) => AllNewProducts(
                                          category: product
...
class _ModalSheetCartState extends State<ModalSheetCart> {
  ...
  void addCart(ProductModel product) async {
    ...
    snackBar(context, message: "Product successfully added to your chart.");
  }
  ...
  /*calculate price total item*/
  calculatePriceTotal(ProductModel product) {
    setState(() {
      product.priceTotal =
          (product.cartQuantity * int.parse(product.productPrice));
    });
    addCart(product);
  }
  ...
  Widget build(BuildContext context) {
    return Container(
      height: attributes.isNotEmpty ? 250.h : 120.h,
      width: double.infinity,
      child: Stack(
        children: [
          Column(
                      ...
                      onPressed: load
                          ...
                          : () {
                              if (widget.product.productStock != null &&
                                  widget.product.productStock != 0) {
                                calculatePriceTotal(widget.product);
                              } else {
                                Navigator.pop(context);
                                snackBar(context,
